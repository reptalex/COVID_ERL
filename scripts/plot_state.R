plot_state <- function(x,US_states=NULL,US_counties=NULL,rDlag=11,max_date=Inf){
  ...
  Y[,growth_rate:=shift(growth_rate,rDlag),by=c('state','county')]
  ...
  NYC[,growth_rate:=shift(growth_rate,rDlag)]
  ...
  plot_list[[4]] <- ggplot(Y,aes(deaths_pc,growth_rate))+
    ...
    geom_point(data=X[state==x][.N],cex=8,col='darkgreen')+
    geom_line(data=X[state==x],lwd=2,col='darkgreen')+
    geom_line(data=Y[county==mx[which.max(pop),county]],lwd=2,col='green')+
    geom_point(data=Y[date==max_date & county==mx[which.max(pop),county]],cex=5,col='green')+
